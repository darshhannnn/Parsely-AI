        ...
        while i < len(lines):
            ...
            if line.count('|') >= 2:  # Likely a table row
                ...
                if table_end > table_start:  # Multi-row table
                    ...
                    # Don't count separator rows in row_count
                    row_count = sum(1 for row in table_rows if not re.match(r'^\s*\|?[:\s-]+\|?[:\s-]+\|?[:\s-]*\|?\s*$', row))
...
class SemanticChunker:
    ...
    def _parse_boundary_positions(self, response: str, content_length: int) -> List[int]:
        ...
        # Extract numbers from response, including negative ones to filter them out
    ...
    def chunk_content(self, content: str, document_id: str, document_metadata: Optional[Dict[str, Any]] = None) -> List[ContentChunk]:
        ...
        try:
            ...
            if self.config.strategy == ChunkingStrategy.SEMANTIC:
                ...
                # This handles both unsupported enum values and the "mocked" string case from tests
                raise ChunkingError(f"Unsupported chunking strategy: {self.config.strategy}", document_id)
            
            # Fallback: if no chunks were generated by a complex strategy, use fixed_size
            if not chunks and content.strip():
                self.logger.info("Complex strategy generated no chunks, falling back to fixed_size", document_id=document_id)
                chunks = self._fixed_size_chunking(content, document_id)
            ...
            # Re-raise ChunkingError directly
            raise
        except Exception as e:
    ...
    def _fixed_size_chunking(self, content: str, document_id: str) -> List[ContentChunk]:
        ...
        while start_pos < len(content):
            ...
            if self.config.preserve_sentences and end_pos < len(content):
                adjusted_end = self._find_sentence_boundary(content, end_pos, backward=True)
            ...
            next_start = end_pos - self.config.overlap_size
